	...
	<div style="display: inline-block;">
		...
		>
			...
			>
				...
				<el-form-item label="解说标题" prop="title">
					<el-input
						placeholder="解说标题"
				...
				<el-form-item label="标签">
					<span
						v-if="
							commentaryData.tag_info != null && commentaryData.tag_info.length == 0
						"
						>无</span
					>
					<div>
						<el-tag
							type="success"
							v-for="tag in commentaryData.tag_info"
							:key="tag.id"
							effect="dark"
				...
				<el-form-item label="影片名称">
					...
					<i
						class="el-icon-edit"
						v-if="!isEdit && (commentaryType == 0 || commentaryType == 3)"
				...
				<el-form-item label="资源">
					...
					<el-table :data="commentaryData.source_info_arr" border style="width: 100%;">
						<el-table-column prop="video_oss_audit_path" label="源片试播key" />
						<el-table-column prop="source_status" width="200" label="资源状态">
					...
					<el-table
						...
						style="width: 100%;"
					>
						<el-table-column prop="template_type_name" width="100" label="清晰度" />
						<el-table-column prop="media_key" label="资源key" />
						<el-table-column prop="source_status" width="200" label="资源状态">
							<template slot-scope="scope">
						...
						<el-table-column label="操作" align="center" width="80">
							<template slot-scope="scope">
								<el-button
									type="text"
									v-if="sources == scope.row.media_url"
									@click="toPilot('')"
									style="color: red;"
									>关闭</el-button
								>
								<el-button type="text" v-else @click="toPilot(scope.row.media_url)"
									>试播</el-button
								>
							</template>
						</el-table-column>
						...
						>
							<el-form-item label="原因" :key="1" prop="audit_reason">
								<el-input
									...
									type="textarea"
									resize="none"
									:placeholder="commentaryType == 0 && '请填写打回原因'"
		...
		<el-dialog
			title="工作流状态"
			:visible.sync="visibleDialogWorkflow"
			width="70%"
			@close="workflowList = []"
		>
			<el-table :data="workflowList" border :cell-style="setCellStyle">
				<el-table-column prop="workflow_status" label="工作流状态">
					<template slot-scope="scope">
						{{
							scope.row.workflow_status == "NOT_DO"
								? "提取打回"
								: "" || scope.row.workflow_status == "RELEASE_CANCEL"
								? "重新策划"
								: "" || scope.row.workflow_status == "RELEASE_FAIL"
								? "发布打回"
								: "" || scope.row.workflow_status == "RELEASE_PASS"
								? "发布成功"
								: "" || scope.row.workflow_status == "FAIL_AUDIT"
								? "策划打回"
								: "" || scope.row.workflow_status == "IN_AUDIT"
								? "待发布"
								: "" || scope.row.workflow_status == "IN_DO"
								? "策划中"
	...
	methods: {
		...
		toPilot(src) {
			...
			this.$set(
				...
				"type",
				type.length > 0 && type.indexOf("m3u8") >= 0 ? "application/x-mpegURL" : "video/mp4"
			);
			...
			if (src != "") {
				if (player) {
					player.src({
						type:
							src.length > 0 && src.indexOf("m3u8") >= 0
		...
		getDetail({ commentary_id, commentary_type }) {
			getMovieCommentaryDetail({
				commentary_id,
				commentary_type
			}).then(res => {
				if (res.code == "000000") {
					...
					this.$set(this.commentaryData, "source_info_arr", [
						res.content.source_info || {}
					]);
					this.$set(
						this.commentaryData,
						"transcode_info",
						res.content.source_info.transcode_info || []
					);
